 ...
 */
public class sWindow extends Table {
    ...
    public sWindow(String title, Skin skin,ShaderProgram shader) {
    ...
    public sWindow(String title, Skin skin, String styleName) {
        this(title, skin.get(styleName, WindowStyle.class));
        setSkin(skin);
    }

    public sWindow(String title, WindowStyle style) {
    ...
    protected void drawBackground (Batch batch, float parentAlpha, float x, float y) {
        ...
        titleCache.setColors(getColor().mul(style.titleFontColor));

        if (shader != null) {
            batch.setShader(shader);
            shader.setUniformf("u_smoothing", 0.75f / (4 * scale));
        }
        titleCache.setPosition((int)x, (int)y+6);
    ...
    /** The style for a window, see {@link sWindow}.
     * @author Nathan Sweet */
